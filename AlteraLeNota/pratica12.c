#include <stdio.h>
#include <ctype.h>
#include <stdlib.h>
#include <string.h>

#define MAX_TAM 100

void modificarNotas(char nome_arquivo[]) {
	int num=-1, num_edit;
	char* nome;
	char buf[MAX_TAM];	
	float nota1, nota2;
	FILE *arq, *temp;
	int achei = 0;

	arq = fopen(nome_arquivo, "r");

	if (arq == NULL) // se arquivo nao foi aberto corretamente 
	{
		printf("\nerro ao abrir o arquivo!");
		return; 
	}
	
	printf("\nDigite o numero do aluno a modificar: ");
	scanf("%d", &num_edit);
	
	printf("\nProcurando...");
	
	temp = fopen("temp.txt", "w");  // abre arq temporario
	
	fgets(buf, MAX_TAM, arq);

	while(!feof(arq)) // enquanto nao for o final do arquivo
	{
		num = atoi(strtok(buf, ","));
		nome = strtok(NULL, ",");
		nota1 = atof(strtok(NULL, ","));
		nota2 = atof(strtok(NULL, ","));
		
		//verificar se "num" eh igual a "num_edit":
		if (num == num_edit)
		{
			printf("\nDigite as duas novas notas do aluno:\n");
			scanf("%f %f", &nota1, &nota2);
			achei = 1;
		}
					
		//escreve os dados lidos do arquivo "nome_arquivo" no arquivo temporario
		fprintf(temp, "%d, %s, %4.1f, %4.1f\n", num, nome, nota1, nota2);
		
		fgets(buf, MAX_TAM, arq);
	}
	
	fclose(arq);
	fclose(temp);
	
	if(achei == 0)
		printf("\nAluno nao encontrado!");

	remove(nome_arquivo);
	rename("temp.txt", nome_arquivo);

}

void leAluno(char nome_arquivo[], int num_alunos) 
{
	int num;
	char nome[20];
	float nota1, nota2;
	FILE *arq;
	arq = fopen(nome_arquivo,"a");
	
	if(arq == NULL) 
	{
		printf("\nErro ao abrir o arquivo!");
		return;
	}

	for (int i = 0; i < num_alunos; i++)
	{	
		printf("\nDigite os dados do novo aluno");
		printf("\nNum: ");
		scanf("%d", &num);
		printf("\nNome: ");
		fflush(stdin);
		fgets(nome, MAX_TAM, stdin); 
		printf("\nNota1: ");
		scanf("%f", &nota1);
		printf("\nNota2: ");
		scanf("%f", &nota2);	
		
		int j = 0;
		while (nome[j] != '\n' && nome[j] != '\0')
			j++;

		nome[j] = '\0';
		fprintf(arq, "%d, %s, %4.1f, %4.1f\n", num, nome, nota1, nota2);
	}

	fclose(arq);
	
}

void imprimeNotas(char nome_arquivo[]) {
	float notas, soma;
	char buf[MAX_TAM];
	int num;
	char *nome;
	float nota1, nota2;
	FILE *arq;

	arq = fopen(nome_arquivo,"r");

	if(arq == NULL) {
		printf("\nErro ao abrir o arquivo!");
		return;
	}

	printf("\nmatricula\t nome\t\t nota1\t nota2\t");

	notas = 0;
	soma = 0;

	fgets(buf, MAX_TAM, arq);

	while(!feof(arq))
	{
		num = atoi(strtok(buf, ","));
		nome = strtok(NULL, ",");
		nota1 = atof(strtok(NULL, ","));
		nota2 = atof(strtok(NULL, ","));
		printf("\n%d \t %20s \t %4.1f \t %4.1f", num, nome, nota1, nota2);
		notas = notas + 2;
		soma = soma + nota1 + nota2;
		fgets(buf, MAX_TAM, arq);
	}
	printf("\nmedia = %f",soma/notas);
	fclose(arq);
}

int main(int argc, char *argv[]) {

	int i;
	char nome_arquivo[10] = "GAAL.txt";
	char exibe_media = 'N';
	char edita_aluno = 'N';
	int le_alunos = 0;
	
	for(i=0; i<argc; i++) 
	{
		if(strcmp(argv[i], "-d") == 0) //strcmp compara parametro 1 com 2, se igual retorna 0
		{
			//coloca o parametro seguinte na variavel nome_arquivo e acrescenta a extensao .txt:
			strcpy(nome_arquivo, argv[i+1]); 
			strcat(nome_arquivo, ".txt"); 
		}

		else if(strcmp(argv[i], "-m") == 0) 
		{
			//coloca o primeiro caractere (o segundo eh o \0) do parametro seguinte na variavel exibe_media:
			exibe_media = argv[i+1][0];
		}
		
		else if(strcmp(argv[i], "-e") == 0)
		{
			edita_aluno = 'S';
		}

		else if (strcmp(argv[i], "-l") == 0)
		{ // se eu quiser adicionar x alunos coloco -l x
			//transforma o parametro seguinte em inteiro a partir da funcao atoi, atribuindo ele aa variavel le_alunos:
			le_alunos = atoi(argv[i+1]);
		}

		else if(argc == 1 || strcmp(argv[i], "-help") == 0 || argv[i][0] == '?') 
		{
			printf("\nParametros do programa:");
			printf("\n-d [XXX] ..... Sigla da disciplina. Ex: -d AEDS1");
			printf("\n-m [S ou N] ..... Exibir media. Ex: -m S");
			printf("\n-l [#] ..... Le informacao sobre # alunos. Ex: -l 4");				
			printf("\n-e ..... Modifica nota de aluno.");			
			return 1;		
		}
	}
	
	if(le_alunos > 0)
		leAluno(nome_arquivo, le_alunos);

	if(edita_aluno == 'S')
		modificarNotas(nome_arquivo);

	if(exibe_media == 'S')
		imprimeNotas(nome_arquivo);
	
	return 0;
}